...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(message: String, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        let url = "https://api.twitter.com/1.1/statuses/update.json"
        TwitterAPICaller.client?.post(url, parameters: ["status": message], progress: nil, success: { (task: URLSessionTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
